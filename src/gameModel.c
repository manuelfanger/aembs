...
void Game_SetScore(uint16_t score_to_add) {
  taskENTER_CRITICAL();
...
bool Game_CheckIfGameWon(void) {
  return (game.player!=NULL && game.diamond==NULL);
}
...
bool Game_CheckCollisions(void) {
  ...
  changed |= Sprite_CheckCollision(&game.monster, false, &game.player, 0, NULL, boom0, boom1); /* check if monster touches the player: destroys player and game is lost */
  changed |= Sprite_CheckCollision(&game.boulder, false, &game.player, 0, NULL, boom0, boom1); /* check if boulder falls on player: game is lost */
  changed |= Sprite_CheckCollision(&game.player, false, &game.diamond, GAME_SCORE_COLLECTED_DIAMOND, NULL, boom0, boom1); /* check if player touches diamond: bingo! */
  changed |= Sprite_CheckCollision(&game.boulder, true, &game.monster, GAME_SCORE_DESTROYED_MONSTER, MonsterToDiamondCb, boom0, boom1); /* check if boulder touches monster: destroys monster */
  return changed;
}
...
void Game_NextLevel(uint8_t level) {
  Boulder_CreateLevel(level); /* player will be created here too */
}
